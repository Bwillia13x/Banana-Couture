...
const SupplyChainVisualizer: React.FC<{ map: SupplyChainMap }> = ({ map }) => {
    // Determine positions on a 600x300 canvas
    // Nodes: Raw -> Processing -> Manufacturing -> Distribution
    const nodes = map.nodes.map((node, i) => {
        // Distribute horizontally based on type order roughly
        let x = 100;
        if (node.type === 'processing') x = 250;
        if (node.type === 'manufacturing') x = 400;
        if (node.type === 'distribution') x = 550;
        
        // Add some jitter for multiple nodes of same type
    ...
    return (
        <div className="bg-[#0F172A] rounded-xl overflow-hidden shadow-inner border border-slate-700 relative h-[320px] w-full">
            <svg width="100%" height="100%" viewBox="0 0 650 300" className="absolute inset-0">
                ...
                {/* Connections */}
                {nodes.map((source, i) => {
                    // Simple logic: connect to next stage nodes
                    // Ideally we use the 'connections' array from the data, but for visual simplicity we can just link left-to-right logic or use the explicit links if IDs match
                    // Using map.connections to draw lines
...
export const FashionForge: React.FC<FashionForgeProps> = ({
  ...
  const [techPackLanguage, setTechPackLanguage] = useState('English');
  ...
  const handleFindManufacturers = async () => {
    ...
    try {
      const results = await matchManufacturers(
        conceptImage,
        cadImage,
        bomMarkdown,
        matchPreferences
  ...
  const handleGenerateTechPack = async () => {
    ...
    try {
      const pack = await generateFactoryTechPack(
        conceptImage,
        cadImage,
        bomMarkdown,
        techPackLanguage,
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-bold ${
          ...
          match.matchScore >= 60 ? 'bg-amber-100 text-amber-700' :
          'bg-nc-bg-soft text-nc-ink-soft'
        }`}>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {match.manufacturer.certifications.slice(0, 3).map((cert, i) => (
          <span key={i} className="px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[9px] rounded-full font-medium border border-emerald-100">
            {cert}
          </span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-nc-bg-soft rounded-lg p-2">
          <div className="text-[10px] text-nc-ink-subtle uppercase">Cost</div>
          <div className="text-sm font-bold text-nc-ink">${match.estimatedCost.perUnit.toFixed(2)}/u</div>
        </div>
        <div className="bg-nc-bg-soft rounded-lg p-2">
          <div className="text-[10px] text-nc-ink-subtle uppercase">Lead Time</div>
          <div className="text-sm font-bold text-nc-ink">{match.estimatedTimeline.totalDays}d</div>
        </div>
        <div className="bg-nc-bg-soft rounded-lg p-2">
          <div className="text-[10px] text-nc-ink-subtle uppercase">Eco</div>
          <div className="text-sm font-bold text-emerald-600">{match.manufacturer.sustainabilityScore}/100</div>
        </div>
      </div>
      
      {match.matchReasons.length > 0 && (
        <div className="mt-3 pt-3 border-t border-nc-border-subtle">
          <p className="text-[10px] text-nc-ink-subtle uppercase mb-1">Why this match</p>
          <ul className="space-y-0.5">
            {match.matchReasons.slice(0, 2).map((reason, i) => (
              <li key={i} className="text-xs text-nc-ink-soft flex items-center gap-1">
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="bg-nc-bg-elevated rounded-xl p-4 border border-nc-border-subtle text-center">
          ...
          <div className="text-2xl font-black text-nc-rose font-display">${costs.margin.suggestedRetail.toFixed(2)}</div>
        </div>
      </div>

      {/* Detailed Breakdown */}
      <div className="bg-nc-bg-elevated rounded-xl p-4 border border-nc-border-subtle">
        <h4 className="text-xs font-bold text-nc-ink-subtle uppercase mb-4 tracking-widest">Cost Breakdown</h4>
        
        <div className="space-y-4">
          {/* Materials */}
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-nc-ink-soft">Materials</span>
              <span className="text-sm font-bold text-nc-ink">
                ${(costs.materials.fabric + costs.materials.trims + costs.materials.hardware + costs.materials.packaging).toFixed(2)}
              </span>
            </div>
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(costs.materials).map(([key, value]) => (
                <div key={key} className="text-center p-2 bg-nc-bg-soft rounded-lg">
                  <div className="text-[9px] text-nc-ink-subtle uppercase">{key}</div>
                  <div className="text-xs font-bold text-nc-ink">${(value as number).toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-nc-ink-soft">Labor</span>
              <span className="text-sm font-bold text-nc-ink">
                ${(costs.labor.cutting + costs.labor.sewing + costs.labor.finishing + costs.labor.qc).toFixed(2)}
              </span>
            </div>
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(costs.labor).map(([key, value]) => (
                <div key={key} className="text-center p-2 bg-blue-50/50 rounded-lg">
                  <div className="text-[9px] text-blue-500 uppercase">{key}</div>
                  <div className="text-xs font-bold text-blue-700">${(value as number).toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-nc-ink-soft">Overhead</span>
              <span className="text-sm font-bold text-nc-ink">
                ${(costs.overhead.sampling + costs.overhead.shipping + costs.overhead.duties + costs.overhead.miscellaneous).toFixed(2)}
              </span>
            </div>
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(costs.overhead).map(([key, value]) => (
                <div key={key} className="text-center p-2 bg-amber-50/50 rounded-lg">
                  <div className="text-[9px] text-amber-600 uppercase">{key}</div>
                  <div className="text-xs font-bold text-amber-800">${(value as number).toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-nc-border-subtle">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-nc-ink-soft">Profit Margin (Retail)</span>
            <span className="text-lg font-black text-emerald-600 font-display">{costs.margin.profitMargin.toFixed(1)}%</span>
          </div>
          <div className="mt-2 h-2 bg-nc-bg-soft rounded-full overflow-hidden">
            <div 
              className="h-full bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-full"
              style={{ width: `${Math.min(100, costs.margin.profitMargin)}%` }}
            />
          </div>
        </div>
      </div>
  ...
  const renderTimeline = (timeline: ProductionTimeline) => (
    <div className="bg-nc-bg-elevated rounded-xl p-4 border border-nc-border-subtle">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-sm font-bold text-nc-accent">
          Est. Delivery: {timeline.estimatedDeliveryDate}
        </div>
      </div>
          ...
          {timeline.phases.map((phase, idx) => (
            <div key={idx} className="relative pl-10">
              ...
              <div className={`absolute left-2 w-4 h-4 rounded-full border-2 ${
                idx === 0 ? 'bg-nc-accent border-nc-accent' : 'bg-nc-bg-elevated border-nc-border-strong'
              }`}></div>
              ...
              <div className="bg-nc-bg-soft rounded-lg p-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 bg-nc-border-subtle rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-nc-accent rounded-full"
                      style={{ width: `${(phase.durationDays / timeline.totalDays) * 100}%` }}
                    />
                  </div>
      ...
      <div className="mt-4 pt-4 border-t border-nc-border-subtle flex justify-between items-center">
        <div>
          <span className="text-[10px] text-nc-ink-subtle uppercase">Total Production</span>
          <div className="text-lg font-bold text-nc-ink">{timeline.totalDays} days</div>
        </div>
        <div className="text-right">
          <span className="text-[10px] text-nc-ink-subtle uppercase">Buffer Included</span>
          <div className="text-lg font-bold text-amber-600">+{timeline.bufferDays} days</div>
        </div>
      </div>
    </div>
  );
  ...
  const renderSupplyChain = () => {
    ...
    return (
      <div className="space-y-6">
            ...
            {supplyChain.nodes.map((node, idx) => (
              <div key={node.id} className="flex items-center gap-4 p-3 bg-nc-bg-soft rounded-lg">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-bold ${
                  node.type === 'raw_material' ? 'bg-amber-500' :
                  node.type === 'processing' ? 'bg-blue-500' :
                  node.type === 'manufacturing' ? 'bg-purple-500' :
                  'bg-emerald-500'
                }`}>
                  {idx + 1}
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-sm font-bold text-nc-ink">{node.name}</div>
                      <div className="text-xs text-nc-ink-soft">{node.location.city || ''} {node.location.country}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-nc-ink-subtle">{node.sustainabilityMetrics.carbonFootprint} kg CO2</div>
                      <div className="text-xs text-blue-500">{node.sustainabilityMetrics.waterUsage} L water</div>
                    </div>
                  </div>
                  {node.sustainabilityMetrics.certifications.length > 0 && (
                    <div className="flex gap-1 mt-1">
                      {node.sustainabilityMetrics.certifications.map((cert, i) => (
                        <span key={i} className="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 text-[8px] rounded border border-emerald-200">
                          {cert}
                        </span>
                      ))}
                    </div>
        ...
        {/* Suggestions */}
        {supplyChain.suggestions.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <h4 className="text-xs font-bold text-amber-700 uppercase mb-2 tracking-widest">Sustainability Suggestions</h4>
            <ul className="space-y-1">
              {supplyChain.suggestions.map((suggestion, i) => (
                <li key={i} className="text-xs text-amber-900 flex items-start gap-2">
                  <span className="text-amber-600">→</span> {suggestion}
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="bg-nc-bg rounded-2xl shadow-2xl border border-nc-border-subtle w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-nc-bg-elevated border-b border-nc-border-subtle p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-nc-ink font-display">FashionForge</h2>
              <p className="text-xs text-nc-ink-subtle tracking-wide">AI-Powered Production Pipeline</p>
            </div>
          </div>
          
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-nc-bg-soft hover:bg-nc-border-strong text-nc-ink-subtle hover:text-nc-ink flex items-center justify-center transition-all">
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-all whitespace-nowrap ${
                  activeTab === tab.id
                    ? 'border-nc-accent text-nc-accent-strong bg-nc-bg-soft/50'
                    : 'border-transparent text-nc-ink-subtle hover:text-nc-ink hover:bg-nc-bg-soft'
                }`}
              >
                <span className="mr-2 opacity-70">{tab.icon}</span> {tab.label}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Loading Overlay */}
        {isLoading && (
          <div className="absolute inset-0 bg-nc-bg-elevated/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
            <div className="w-12 h-12 border-4 border-nc-border-strong border-t-nc-accent rounded-full animate-spin mb-4"></div>
            <p className="text-sm font-medium text-nc-ink-soft animate-pulse">{loadingMessage}</p>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-nc-bg">
          {activeTab === 'match' && (
            <div className="space-y-6">
                <div className="bg-nc-bg-elevated rounded-xl p-6 border border-nc-border-subtle shadow-sm">
                    <button onClick={handleFindManufacturers} disabled={isLoading} className="w-full py-3 bg-nc-ink text-white font-bold rounded-xl">Find Manufacturers</button>
                </div>
                {matches.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">{matches.map(renderMatchCard)}</div>
                        {selectedMatch && (
                            <div className="bg-nc-bg-elevated rounded-xl p-6 border border-nc-border-subtle shadow-md">
                                <h3 className="font-bold mb-2">{selectedMatch.manufacturer.name}</h3>
                                <button onClick={handleMapSupplyChain} className="w-full py-2 bg-emerald-600 text-white rounded-lg mb-2">Map Supply Chain</button>
                            </div>
                        )}
                    </div>
                )}
              ...
              <div className="space-y-6">
                  {!techPack ? (
                      <div className="text-center p-8">
                          <button onClick={handleGenerateTechPack} className="px-8 py-3 bg-nc-accent text-white font-bold rounded-xl">Generate Tech Pack</button>
                      </div>
                  ) : renderTechPack()}
              </div>
          )}
